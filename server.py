...
from tools.checkout import QR_DIR
...
async def qr_image(filename: str):
    """Serves a Blinkit UPI QR screenshot generated by checkout()'s
    payment_method="upi" path (tools/checkout.py writes these under
    tools/.qr_codes/) so the web console can render checkout's
    qr_image_path — a local filesystem path — as an <img src>. Only ever
    serves a bare filename resolved against QR_DIR, never an arbitrary
    path, so a `filename` like "../../.env" can't escape that directory."""
    path = (QR_DIR / filename).resolve()
    if QR_DIR.resolve() not in path.parents or not path.is_file():
